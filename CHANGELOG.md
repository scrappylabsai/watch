...
- **Transcript injection banner (`watch.py`, `emit_untrusted_banner`)** — every transcript now goes through `scan-untrusted` and the verdict is printed into the report itself (STDOUT, because the report *is* what the model reads). A transcript is third-party prose landing straight in agent context; that's the exact path a real injection took on 2026-07-24 (`Projects/Security/INJECTION-SAMPLES.md`).
  - Fails **open** on any tooling error — a broken scanner must never stop someone watching a video — but fails **loud**: the banner always states what actually happened, so "no warning" can never be misread as "checked and clean".
  - Distinguishes the **two causes of `UNGATED`**, which are not the same problem: (1) AgentWorld is genuinely down, vs (2) bulk content exceeded `--max-windows` so the gate deliberately declined to read 6 arbitrary windows out of 20. **A long video hits case 2 with the gate up and perfectly healthy** — reporting it as "the model is down" sends you chasing a non-existent outage. Case 2 prints the `aw scan --content` command to actually scan the whole thing.

...
# Local-first ASR: ~/bin/listen wraps a fleet Qwen3-ASR endpoint with cloud fallback.
if command -v listen >/dev/null 2>&1 \
   || [[ -x "$HOME/bin/listen" ]] \
   || [[ -x "/usr/local/bin/listen" ]] \
   || [[ -x "/opt/homebrew/bin/listen" ]]; then
  HAS_LISTEN="yes"
fi
...
elif [[ -z "$HAS_LISTEN" && -z "$HAS_GROQ" && -z "$HAS_OPENAI" ]]; then
  echo "/watch: ready for videos with native captions. Install ~/bin/listen for local-first ASR, or add GROQ_API_KEY / OPENAI_API_KEY to ~/.config/watch/.env for cloud Whisper fallback."
else
  echo "/watch: ready."
fi
